...
async function getPosts(): Promise<Post[]> {
  try {
    ...
    const response = await fetch('https://jsonplaceholder.typicode.com/posts', { cache: 'no-store' }); // Disable caching for fresh data
    if (!response.ok) {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 flex-grow">
        <header className="mb-8 sm:mb-12 text-center">
          ...
          <p className="mt-3 text-md sm:text-lg text-muted-foreground max-w-2xl mx-auto">
            Discover a collection of captivating posts. Click on any card to learn more (and trigger a GA event!).
          </p>
        </header>
        ...
        {posts.length > 0 ? (
          ...
          <Alert variant="default" className="max-w-lg mx-auto">
            ...
            <AlertDescription>
              We couldn&apos;t retrieve any posts at the moment. Please try again later.
            </AlertDescription>
          </Alert>
        )}
      </div>
      <footer className="py-6 text-center text-sm text-muted-foreground border-t border-border">
        <p>&copy; {new Date().getFullYear()} Scroll Insights. All rights reserved.</p>
      </footer>
    </main>
  );
}
